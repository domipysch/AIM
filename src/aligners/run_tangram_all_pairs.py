"""Batch runner: Tangram alignment + metrics for every pair × cell-type granularity.

Run from the repository root with the tangram_env active:
    conda activate tangram_env
    python -m src.aligners.run_tangram_all_pairs [options]
...
parser = argparse.ArgumentParser(
    description="Run Tangram alignment and metrics for every pair in pairs.csv, "
...
parser.add_argument(
    "--normalize_and_log",
    action="store_true",
    help="Normalize and log-transform input data before alignment",
)
parser.add_argument(
    "--compute_marker_genes",
    action="store_true",
    help="Compute marker genes before mapping (slower but recommended by Tangram)",
)
parser.add_argument(
    "--permutation_tests",
    action="store_true",
...
for pair in pairs:
    ...
    for ct_key in keys:
        ...
        if skip_existing and gep_prob_path.exists() and gep_det_path.exists():
            logger.info(f"{tag} Skipping Tangram — GEP outputs already exist")
        else:
            logger.info(f"{tag} Running Tangram: {sc_name}  ×  {st_name}")
            try:
                tangram_align_data(
                    sc_path=sc_path,
                    st_path=st_path,
