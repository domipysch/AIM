"""Batch runner: TACCO alignment + metrics for every pair × cell-type granularity.

Run from the repository root with the tacco_env active:
    conda activate tacco_env
    python -m src.aligners.run_tacco_all_pairs [options]
...
parser = argparse.ArgumentParser(
    description="Run TACCO alignment and metrics for every pair in pairs.csv, "
...
for pair in pairs:
    ...
    for ct_key in keys:
        ...
        if skip_existing and gep_prob_path.exists() and gep_det_path.exists():
            logger.info(f"{tag} Skipping TACCO — GEP outputs already exist")
        else:
            logger.info(f"{tag} Running TACCO: {sc_name}  ×  {st_name}")
            try:
                tacco_align_data(
                    sc_path=sc_path,
                    st_path=st_path,
