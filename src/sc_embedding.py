...
def _compute_pca(adata_sc: AnnData, d: int) -> np.ndarray:
    """Normalise → log1p → select top 2000 HVGs → PCA. Returns (C, d) float32 array."""
    import scanpy as sc
...
def _compute_scvi(
    ...
    """
    Train scVI (200 epochs) and return the latent representation as (C, d) float32 array.

    When cache_dir is provided the result is read from / written to
    cache_dir/sc-scvi.h5ad, keyed by the requested dimension d.
    If the cached file exists but was computed with a different d, it is recomputed.
    """
